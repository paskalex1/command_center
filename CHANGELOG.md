...
- Document Collector сохраняет результаты Web Knowledge MCP в `docs/<project>/web_knowledge/…`, запускает `ingest_project_docs`, и свежие материалы сразу доступны через RAG Librarian.
- Лимит вызовов инструментов теперь задаётся настройкой `AGENT_TOOL_MAX_ITERATIONS` (дефолт 25, `0` — отключить), а сообщение о лимите появляется только если ограничение включено.
