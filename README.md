...
- Лимит цепочек инструментов на одно сообщение регулируется переменной окружения `AGENT_TOOL_MAX_ITERATIONS` (по умолчанию 25, `0` — отключить лимит).
- Агенты вроде **Document Collector** используют Web Knowledge MCP (`search_and_collect_knowledge`). Сохранённые материалы автоматически пишутся в `docs/<project>/web_knowledge/` и сразу ставят `ingest_project_docs` в очередь — поэтому RAG Librarian видит новые знания без ручных действий.
